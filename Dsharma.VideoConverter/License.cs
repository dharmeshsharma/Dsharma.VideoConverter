...
namespace Dsharma.VideoConverter
{
	...
	//				return;
	//			}
	//			throw new Exception("Commercial license key is required (https://www.Dsharmasite.com/video_converter_net.aspx)");
	//		}
	...
	//			byte[] publicKey = typeof(LicenseInternal).GetTypeInfo().get_Assembly().GetName()
	//				.GetPublicKey();
	//			if (publicKey == null)
	//			{
	//				throw new Exception("Dsharma.VideoConverter.LT is not strongly signed");
	//			}
	//			byte[] bytes = Encoding.UTF8.GetBytes(I.Owner);
	//			using (RSA rSA = RSA.Create())
	//			{
	//				RSAParameters publicKeyRSAParameters = GetPublicKeyRSAParameters(publicKey);
	//				rSA.ImportParameters(publicKeyRSAParameters);
	//				try
	//				{
	//					if (!rSA.VerifyData(bytes, licenseKeyBytes, HashAlgorithmName.SHA1, RSASignaturePadding.Pkcs1))
	//					{
	//						throw new Exception();
	//					}
